...
namespace WebApi.Controllers
{
    ...
    public class BlogsController : BaseController
    {
        ...
        public BlogsController(IBlogService blogService, IMapper mapper, ICommentDal commentDal, IMemoryCache memoryCache)
        {
        ...
        {
            if (_memoryCache.TryGetValue("blogList", out List<BlogListDtos> list))
            {
                return Ok(list);
            }

            var blogList = _mapper.Map<List<BlogListDtos>>(await _blogService.GetAllSortedByPostedTimeAsync());
        ...
        {
            return Ok(_mapper.Map<BlogListDtos>(await _blogService.FindByIdAsync(id)));
        }
       ...
       [HttpPost("[action]")]
        ...
        {
            return Ok(_mapper.Map<List<BlogListDtos>>(await _blogService.GetLastFiveAsync()));
        }
        ...
        public async Task<IActionResult> GetComments([FromRoute]int id, [FromQuery]int? parentCommentId)
        {
            return Ok(_mapper.Map<List<CommentListDto>>(
                await _commentDal.GetAllWithSubCommentsAsync(id, parentCommentId)));
        }
        ...
        {
            return Ok(_mapper.Map<List<BlogListDtos>>(await _blogService.SearchAsync(s)));
        }
        ...
        public async Task<IActionResult> AddComment(CommentAddDto commentAddDto)
        {
           ...
           await _commentDal.AddAsync(_mapper.Map<Comment>(commentAddDto));
